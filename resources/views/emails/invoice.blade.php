...
<p>Pay from the ETH address: {{ $order->to_address }}</p>
    ...
    src="{!!
        $message->embedData(
            QrCode::format('png')
                ...
                ->errorCorrection('H')
                ->generate('ethereum:' . \str_start($order->from_address, '0x')),
            'QrCode.png',
            'image/png'
        )
    !!}"
>
<br>
<small>Ethereum Address: <strong>{{ $order->from_address }}</strong></small>
<br>
